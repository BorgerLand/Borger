...
///|**Example Use Cases**       |Processing client inputs (movement, shooting, etc.) should happen here in Immediate as much as possible for best game feel. Controls are by far the most latency-sensitive aspect of gameplay.                                                                                                                               |Logic that affects entities seen by clients but not controlled by clients due to private state (NPC)                                                                                                                                                         |Large/important game state change events that would look horrible if rolled back/mispredicted (game over, level change)                                                            |
///
///## Parameters
...
///			//(should be 1-2 seconds absolute worst case). we want to
///			//avoid someone seeing "you win!" then a second later their
///			//screen changes to "you lose!" at all costs
///		}
///	});
///}
///
