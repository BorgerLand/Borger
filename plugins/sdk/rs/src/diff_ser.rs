...
use crate::primitive::{PrimitiveSerDes, SliceSerDes, usize32};
use crate::{DiffOperation, TickType};
...
impl DiffSerializer<Impl> {
	...
	pub fn get_rollback_buffer(&mut self) -> &mut Vec<u8> {
		&mut self.rollback_buffer
	}
		...
		if let Some(shared_len) = find_first_mismatch(&tx_data.cur_path, &new_path) {
			...
			if cur_len > shared_len {
				if shared_len == 0 {
					...
					(((cur_len - shared_len) / 2) as u8).ser_tx(buffer);
				}
			}
