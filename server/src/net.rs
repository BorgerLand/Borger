...
pub const NET_CERT_REFRESH_INTERVAL: Duration = Duration::from_hours(24); //only applies to certs loaded from disk, not autogenerated self-signed
pub const NET_CERT_RETRY_INTERVAL: Duration = Duration::from_secs(1); //how often to retry refresh in case of failure
...
pub async fn init(new_connection_sender: SyncSender<AsyncSender<SimToClientCommand>>, flags: Flags) {
	...
	let devcert_contents = identity.certificate_chain().as_slice()[0]
		.hash()
		.fmt(Sha256DigestFmt::BytesArray);

	if let Some(devcert_path) = flags.devcert {
		tokio::spawn(async move {
			if let Some(parent) = devcert_path.parent() {
				fs::create_dir_all(parent).await.unwrap();
			}

			fs::write(devcert_path, devcert_contents.as_bytes())
				.await
				.unwrap()
		});
	}

	let wt_config = wt_build(identity.clone_identity(), flags.webtransport_port);
	let wt_server_tx = Arc::new(Endpoint::server(wt_config).unwrap());
	let wt_server_rx = wt_server_tx.clone();
	let wt_actual_port = wt_server_tx.local_addr().unwrap().port();
	...
	//run the 2 servers and cert refresher concurrently
	tokio::join!(
		async {
			loop {
				let incoming_session = wt_server_rx.accept().await;
		...
		async {
			loop {
				...
				let ws_certs = ws_certs_rx.borrow();
		...
		async {
			//handle certificate renewal by continuously refreshing from disk
			if let Some((fullchain, privkey)) = certs {
				let webtransport_port = flags.webtransport_port;
				let mut refresh_interval = interval(NET_CERT_REFRESH_INTERVAL);
				refresh_interval.set_missed_tick_behavior(MissedTickBehavior::Delay);
				refresh_interval.tick().await; //discard first tick because it resolves immediately

				loop {
					refresh_interval.tick().await;

					let mut retry_interval = interval(NET_CERT_RETRY_INTERVAL);
					retry_interval.set_missed_tick_behavior(MissedTickBehavior::Delay);
					let new_identity = loop {
						retry_interval.tick().await;
						match Identity::load_pemfiles(&fullchain, &privkey).await {
							Ok(id) => break id,
							Err(oops) => {
								error!("Failed to reload certificates from disk: {}", oops);
							}
						}
					};

					let new_wt_config = wt_build(new_identity.clone_identity(), webtransport_port);
					wt_server_tx.reload_config(new_wt_config, false).unwrap(); //can only fail when rebind is true
					ws_certs_tx.send(ws_build(&new_identity)).unwrap(); //can only fail if shutting down anyways

					info!("Certificates refreshed successfully");
				}
			}
		}
	);
...
fn wt_build(identity: Identity, bind_port: u16) -> WTServerConfig {
...
fn ws_build(identity: &Identity) -> TlsAcceptor {
	TlsAcceptor::from(std::sync::Arc::new(
		WSServerConfig::builder()
			...
			.with_single_cert(
				identity
					...
					.collect(),
				PrivateKeyDer::try_from(identity.private_key().secret_der().to_vec()).unwrap(),
			)
			.unwrap(),
	))
}
