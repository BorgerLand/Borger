...
const engine = await Borger.init({ requireWebGL2: true });
const canvas = await UI.init();
const inputHandler = Input.init(canvas, engine.compat.touchscreen.supported);
...
await Borger.present(engine, function presentationLoop(input: Borger.Input, output: Borger.Output) {
	...
	Input.update(inputHandler, input);
